...
#define CK_SAFE_ADD_REF(x)                                                     \
    do {                                                                       \
        if (x) {                                                               \
            (x)->add_ref();                                                    \
        }                                                                      \
    } while (0)
// NOTE the ref assign accounts for the case that lhs == rhs and the
// reference==1: add BEFORE release
#define CK_SAFE_REF_ASSIGN(lhs, rhs)                                           \
    do {                                                                       \
        Chuck_VM_Object* temp = (lhs);                                         \
        (lhs)                 = (rhs);                                         \
        CK_SAFE_ADD_REF(lhs);                                                  \
        CK_SAFE_RELEASE(temp);                                                 \
    } while (0)
#define CK_SAFE_FREE(x)                                                        \
    do {                                                                       \
        if (x) {                                                               \
            free(x);                                                           \
            (x) = NULL;                                                        \
...
#define CK_SAFE_UNLOCK_DELETE(x)                                               \
    do {                                                                       \
        if (x) {                                                               \
            (x)->unlock();                                                     \
            delete (x);                                                        \
...
#define CK_SAFE_UNLOCK_RELEASE(x)                                              \
    do {                                                                       \
        if (x) {                                                               \
            (x)->unlock();                                                     \
            (x)->release();                                                    \
...
#ifndef __arm__
// dedenormal
#define CK_DDN_SINGLE(f)                                                       \
    f = (f >= 0 ? ((f > (t_CKSINGLE)1e-15 && f < (t_CKSINGLE)1e15) ?           \
                     f :                                                       \
                     (t_CKSINGLE)0.0) :                                        \
                  ((f < (t_CKSINGLE) - 1e-15 && f > (t_CKSINGLE) - 1e15) ?     \
                     f :                                                       \
                     (t_CKSINGLE)0.0))
#define CK_DDN_DOUBLE(f)                                                       \
    f = (f >= 0 ?                                                              \
...
//-------------------------------------------
// platform: linux
// related macros: __LINUX_ALSA__ __LINUX_PULSE__ __LINUX_OSS__ __LINUX_JACK__
// __UNIX_JACK__
//-------------------------------------------
...
// 1.4.2.0 (ge and spencer) added for legacy windows, as part of switch to
// snprintf
...
a_Program append_program(a_Program program, a_Section section, uint32_t line,
                         uint32_t where);
a_Section new_section_stmt(a_Stmt_List stmt_list, uint32_t line,
                           uint32_t where);
a_Section new_section_func_def(a_Func_Def func_def, uint32_t line,
                               uint32_t where);
a_Section new_section_class_def(a_Class_Def class_def, uint32_t line,
                                uint32_t where);
a_Stmt_List new_stmt_list(a_Stmt stmt, uint32_t line, uint32_t where);
a_Stmt_List append_stmt_list(a_Stmt_List stmt_list, a_Stmt stmt, uint32_t line,
                             uint32_t where);
...
a_Stmt new_stmt_from_while(a_Exp cond, a_Stmt body, uint32_t line,
                           uint32_t where);
a_Stmt new_stmt_from_do_while(a_Exp cond, a_Stmt body, uint32_t line,
                              uint32_t where);
a_Stmt new_stmt_from_until(a_Exp cond, a_Stmt body, uint32_t line,
                           uint32_t where);
a_Stmt new_stmt_from_do_until(a_Exp cond, a_Stmt body, uint32_t line,
                              uint32_t where);
a_Stmt new_stmt_from_for(a_Stmt c1, a_Stmt c2, a_Exp c3, a_Stmt body,
                         uint32_t line, uint32_t where);
a_Stmt new_stmt_from_foreach(a_Exp iter, a_Exp array, a_Stmt body,
                             uint32_t line, uint32_t where);
a_Stmt new_stmt_from_loop(a_Exp cond, a_Stmt body, uint32_t line,
                          uint32_t where);
a_Stmt new_stmt_from_if(a_Exp cond, a_Stmt if_body, a_Stmt else_body,
                        uint32_t line, uint32_t where);
...
a_Exp new_exp_from_binary(a_Exp lhs, ae_Operator oper, a_Exp rhs, uint32_t line,
                          uint32_t where);
a_Exp new_exp_from_unary(ae_Operator oper, a_Exp exp, uint32_t line,
                         uint32_t where);
a_Exp new_exp_from_unary2(ae_Operator oper, a_Type_Decl type, int ctor_invoked,
                          a_Exp ctor_args, a_Array_Sub array, uint32_t line,
                          uint32_t where);
a_Exp new_exp_from_unary3(ae_Operator oper, a_Stmt code, uint32_t line,
                          uint32_t where);
a_Exp new_exp_from_cast(a_Type_Decl type, a_Exp exp, uint32_t line,
                        uint32_t where, uint32_t castPos);
a_Exp new_exp_from_array(a_Exp base, a_Array_Sub indices, uint32_t line,
                         uint32_t where);
a_Exp new_exp_from_array_lit(a_Array_Sub exp_list, uint32_t line,
                             uint32_t where);
a_Exp new_exp_from_func_call(a_Exp base, a_Exp args, uint32_t line,
                             uint32_t where);
a_Exp new_exp_from_member_dot(a_Exp base, c_str member, uint32_t line,
                              uint32_t where, uint32_t memberPos);
a_Exp new_exp_from_postfix(a_Exp base, ae_Operator op, uint32_t line,
                           uint32_t where);
...
a_Exp new_exp_from_if(a_Exp cond, a_Exp lhs, a_Exp rhs, uint32_t line,
                      uint32_t where);
...
a_Exp new_exp_from_vec(a_Vec, uint32_t line,
                       uint32_t where); // ge: added 1.3.5.3
a_Exp new_exp_decl_external(a_Type_Decl type_decl,
                            a_Var_Decl_List var_decl_list, int is_static,
                            int is_const, uint32_t line, uint32_t where);
a_Exp new_exp_decl_global(a_Type_Decl type_decl, a_Var_Decl_List var_decl_list,
                          int is_static, int is_const, uint32_t line,
                          uint32_t where);
a_Exp new_exp_decl(a_Type_Decl type_decl, a_Var_Decl_List var_decl_list,
                   int is_static, int is_const, uint32_t line, uint32_t where);
a_Exp new_exp_from_hack(a_Exp exp, uint32_t line, uint32_t where);
a_Exp new_exp_from_nil(uint32_t line, uint32_t where);
a_Var_Decl_List new_var_decl_list(a_Var_Decl var_decl, uint32_t line,
                                  uint32_t where);
a_Var_Decl_List prepend_var_decl_list(a_Var_Decl var_decl, a_Var_Decl_List list,
                                      uint32_t line, uint32_t where);
a_Var_Decl new_var_decl(c_constr xid, int ctor_invoked, a_Exp ctor_args,
                        a_Array_Sub array, uint32_t line, uint32_t where);
a_Type_Decl new_type_decl(a_Id_List xid, int ref, uint32_t line,
                          uint32_t where);
a_Type_Decl add_type_decl_array(a_Type_Decl type_decl, a_Array_Sub array,
                                uint32_t line, uint32_t where);
a_Arg_List new_arg_list(a_Type_Decl type_decl, a_Var_Decl var_decl,
                        uint32_t line, uint32_t where);
a_Arg_List prepend_arg_list(a_Type_Decl type_decl, a_Var_Decl var_decl,
                            a_Arg_List arg_list, uint32_t line, uint32_t where);
a_Array_Sub new_array_sub(a_Exp exp, uint32_t line, uint32_t where);
a_Array_Sub prepend_array_sub(a_Array_Sub array, a_Exp exp, uint32_t line,
                              uint32_t where);
a_Complex new_complex(a_Exp re, uint32_t line, uint32_t where);
a_Polar new_polar(a_Exp mod, uint32_t line,
                  uint32_t where); // ge: added 1.3.5.3
a_Vec new_vec(a_Exp e, uint32_t line, uint32_t where);

a_Class_Def new_class_def(ae_Keyword class_decl, a_Id_List xid, a_Class_Ext ext,
                          a_Class_Body body, uint32_t line, uint32_t where);
a_Class_Body new_class_body(a_Section section, uint32_t line, uint32_t where);
a_Class_Body prepend_class_body(a_Section section, a_Class_Body body,
                                uint32_t line, uint32_t where);
a_Class_Ext new_class_ext(a_Id_List extend_id, a_Id_List impl_list,
                          uint32_t line, uint32_t where);
a_Class_Def new_iface_def(ae_Keyword class_decl, a_Id_List xid, a_Class_Ext ext,
                          a_Class_Body body, uint32_t line, uint32_t where);
a_Id_List new_id_list(c_constr xid, uint32_t line,
                      uint32_t where /*, YYLTYPE * loc*/);
a_Id_List prepend_id_list(c_constr xid, a_Id_List list, uint32_t line,
                          uint32_t where /*, YYLTYPE * loc*/);
a_Func_Def new_func_def(ae_Keyword func_decl, ae_Keyword static_decl,
                        a_Type_Decl type_decl, c_str name, a_Arg_List arg_list,
                        a_Stmt code, uint32_t is_from_ast, uint32_t line,
                        uint32_t where);
a_Func_Def new_op_overload(ae_Keyword func_decl, ae_Keyword static_decl,
                           ...
                           uint32_t line, uint32_t where, uint32_t operPos);
...
//------------------------------------------------------------------------------
struct a_Exp_Binary_ {
    a_Exp lhs;
    ae_Operator op;
    a_Exp rhs;
    t_CKFUNC ck_func;
    t_CKFUNC ck_overload_func;
    uint32_t line;
    uint32_t where;
    a_Exp self;
};
...
// 1.4.2.0 (ge) added ck_type and ref, to handle multiple array decl (e.g., int
// x, y[], z[1];)
struct a_Var_Decl_ {
    S_Symbol xid;
    struct a_Ctor_Call_ ctor;
...
struct a_Stmt_Switch_ {
    a_Exp val;
    uint32_t line;
    uint32_t where;
    a_Stmt self;
};
struct a_Stmt_Break_ {
    uint32_t line;
    uint32_t where;
    a_Stmt self;
};
struct a_Stmt_Continue_ {
    uint32_t line;
    uint32_t where;
    a_Stmt self;
};
struct a_Stmt_Return_ {
    a_Exp val;
    uint32_t line;
    uint32_t where;
    a_Stmt self;
...
// struct t_Func_BuiltIn_ { builtin_func_ptr func_ptr; uint32_t line; uint32_t
// where; };
struct a_Func_Def_ {
    ...
    uint32_t overload_post;        // 1.5.1.5 (ge) overload as post? e.g.,
                                   // @operator(args) ++ (unary only)
...
#define GET_CK_STRING_SAFE(ptr)                                                \
    std::string(API->object->str((Chuck_String*)ptr))
#define GET_NEXT_STRING_SAFE(ptr)                                              \
    std::string(API->object->str(GET_NEXT_STRING(ptr)))
#endif
...
#define OBJ_MEMBER_FLOAT(obj, offset)                                          \
    (*(t_CKFLOAT*)OBJ_MEMBER_DATA(obj, offset))
...
#define OBJ_MEMBER_VECTOR(obj, offset)                                         \
    (*(t_CKVECTOR*)OBJ_MEMBER_DATA(obj, offset))
#define OBJ_MEMBER_OBJECT(obj, offset)                                         \
    (*(Chuck_Object**)OBJ_MEMBER_DATA(obj, offset))
#define OBJ_MEMBER_STRING(obj, offset)                                         \
    (*(Chuck_String**)OBJ_MEMBER_DATA(obj, offset))
...
#define CK_DLL_DECLVERSION                                                     \
    CK_DLL_EXPORT(t_CKUINT) ck_version()                                       \
    {                                                                          \
        return CK_DLL_VERSION;                                                 \
    }
...
#define CK_DLL_QUERY_STATIC(name)                                              \
    CK_DLL_EXPORT(t_CKBOOL)                                                    \
    CK_DLL_QUERY_STATIC_NAME(name)(Chuck_DL_Query * QUERY)
// macro for defining ChucK DLL export query-functions | example:
// CK_DLL_QUERY(Foo) { ... }
#ifndef __CK_DLL_STATIC__
#define CK_DLL_QUERY(name)                                                     \
    CK_DLL_DECLVERSION CK_DLL_EXPORT(t_CKBOOL) ck_query(Chuck_DL_Query* QUERY)
#else
...
#define CK_DLL_ALLOC(name)                                                     \
    CK_DLL_EXPORT(Chuck_Object*)                                               \
    name(Chuck_VM* VM, Chuck_VM_Shred* SHRED, CK_DL_API API)
// macro for defining ChucK DLL export constructors
// example: CK_DLL_CTOR(foo)
#define CK_DLL_CTOR(name)                                                      \
    CK_DLL_EXPORT(void)                                                        \
    name(Chuck_Object* SELF, void* ARGS, Chuck_VM* VM, Chuck_VM_Shred* SHRED,  \
         CK_DL_API API)
// macro for defining ChucK DLL export destructors
// example: CK_DLL_DTOR(foo)
#define CK_DLL_DTOR(name)                                                      \
    CK_DLL_EXPORT(void)                                                        \
    name(Chuck_Object* SELF, Chuck_VM* VM, Chuck_VM_Shred* SHRED,              \
         CK_DL_API API)
// macro for defining ChucK DLL export member functions
// example: CK_DLL_MFUN(foo)
#define CK_DLL_MFUN(name)                                                      \
    CK_DLL_EXPORT(void)                                                        \
    name(Chuck_Object* SELF, void* ARGS, Chuck_DL_Return* RETURN,              \
...
#define CK_DLL_SFUN(name)                                                      \
    CK_DLL_EXPORT(void)                                                        \
    name(Chuck_Type* TYPE, void* ARGS, Chuck_DL_Return* RETURN, Chuck_VM* VM,  \
         Chuck_VM_Shred* SHRED, CK_DL_API API)
// example: CK_DLL_GFUN(foo) | 1.5.1.5 (ge & andrew) added for global-scope
// function, e.g., for op overloads
#define CK_DLL_GFUN(name)                                                      \
    CK_DLL_EXPORT(void)                                                        \
    name(void* ARGS, Chuck_DL_Return* RETURN, Chuck_VM* VM,                    \
         Chuck_VM_Shred* SHRED, CK_DL_API API)
// macro for defining ChucK DLL export ugen tick functions
// example: CK_DLL_TICK(foo)
#define CK_DLL_TICK(name)                                                      \
    CK_DLL_EXPORT(t_CKBOOL)                                                    \
    name(Chuck_Object* SELF, SAMPLE in, SAMPLE* out, CK_DL_API API)
// macro for defining ChucK DLL export ugen multi-channel tick functions
// example: CK_DLL_TICKF(foo)
#define CK_DLL_TICKF(name)                                                     \
    CK_DLL_EXPORT(t_CKBOOL)                                                    \
    name(Chuck_Object* SELF, SAMPLE* in, SAMPLE* out, t_CKUINT nframes,        \
         CK_DL_API API)
// macro for defining ChucK DLL export ugen ctrl functions
// example: CK_DLL_CTRL(foo)
#define CK_DLL_CTRL(name)                                                      \
    CK_DLL_EXPORT(void)                                                        \
    name(Chuck_Object* SELF, void* ARGS, Chuck_DL_Return* RETURN,              \
         Chuck_VM* VM, Chuck_VM_Shred* SHRED, CK_DL_API API)
// macro for defining ChucK DLL export ugen cget functions
// example: CK_DLL_CGET(foo)
#define CK_DLL_CGET(name)                                                      \
    CK_DLL_EXPORT(void)                                                        \
    name(Chuck_Object* SELF, void* ARGS, Chuck_DL_Return* RETURN,              \
         Chuck_VM* VM, Chuck_VM_Shred* SHRED, CK_DL_API API)
// macro for defining ChucK DLL export ugen pmsg functions
// example: CK_DLL_PMSG(foo)
#define CK_DLL_PMSG(name)                                                      \
    CK_DLL_EXPORT(t_CKBOOL)                                                    \
    name(Chuck_Object* SELF, const char* MSG, void* ARGS, Chuck_VM* VM,        \
         Chuck_VM_Shred* SHRED, CK_DL_API API)
// macro for defining ChucK DLL export uana tock functions
// example: CK_DLL_TOCK(foo)
#define CK_DLL_TOCK(name)                                                      \
    CK_DLL_EXPORT(t_CKBOOL)                                                    \
    name(Chuck_Object* SELF, Chuck_UAna* UANA, Chuck_UAnaBlobProxy* BLOB,      \
         CK_DL_API API)
// macro for defining Chuck DLL export shreds watcher functions
// example: CK_DLL_SHREDS_WATCHER(foo)
#define CK_DLL_SHREDS_WATCHER(name)                                            \
    CK_DLL_EXPORT(void)                                                        \
    name(Chuck_VM_Shred* SHRED, t_CKINT CODE, t_CKINT PARAM, Chuck_VM* VM,     \
         void* BINDLE)
// macro for defining Chuck DLL export type on instantiate functions
// example: CK_DLL_TYPE_ON_INSTANTIATE(foo)
#define CK_DLL_TYPE_ON_INSTANTIATE(name)                                       \
    CK_DLL_EXPORT(void)                                                        \
    name(Chuck_Object* OBJECT, Chuck_Type* TYPE, Chuck_VM_Shred* SHRED,        \
         Chuck_VM* VM)
...
// object
typedef Chuck_Object*(CK_DLL_CALL* f_alloc)(Chuck_VM* VM, Chuck_VM_Shred* SHRED,
                                            CK_DL_API API);
typedef t_CKVOID(CK_DLL_CALL* f_ctor)(Chuck_Object* SELF, void* ARGS,
                                      Chuck_VM* VM, Chuck_VM_Shred* SHRED,
                                      CK_DL_API API);
typedef t_CKVOID(CK_DLL_CALL* f_dtor)(Chuck_Object* SELF, Chuck_VM* VM,
                                      Chuck_VM_Shred* SHRED, CK_DL_API API);
typedef t_CKVOID(CK_DLL_CALL* f_mfun)(Chuck_Object* SELF, void* ARGS,
                                      Chuck_DL_Return* RETURN, Chuck_VM* VM,
                                      Chuck_VM_Shred* SHRED, CK_DL_API API);
// 1.4.1.0 (ge) added TYPE to static prototype
typedef t_CKVOID(CK_DLL_CALL* f_sfun)(Chuck_Type* TYPE, void* ARGS,
                                      Chuck_DL_Return* RETURN, Chuck_VM* VM,
                                      Chuck_VM_Shred* SHRED, CK_DL_API API);
// 1.5.1.5 (ge & andrew) added for global-scope function, e.g., for op overloads
typedef t_CKVOID(CK_DLL_CALL* f_gfun)(void* ARGS, Chuck_DL_Return* RETURN,
                                      Chuck_VM* VM, Chuck_VM_Shred* SHRED,
                                      CK_DL_API API);
// ugen specific
typedef t_CKBOOL(CK_DLL_CALL* f_tick)(Chuck_Object* SELF, SAMPLE in,
                                      SAMPLE* out, CK_DL_API API);
typedef t_CKBOOL(CK_DLL_CALL* f_tickf)(Chuck_Object* SELF, SAMPLE* in,
                                       SAMPLE* out, t_CKUINT nframes,
                                       CK_DL_API API);
typedef t_CKVOID(CK_DLL_CALL* f_ctrl)(Chuck_Object* SELF, void* ARGS,
                                      Chuck_DL_Return* RETURN, Chuck_VM* VM,
                                      Chuck_VM_Shred* SHRED, CK_DL_API API);
typedef t_CKVOID(CK_DLL_CALL* f_cget)(Chuck_Object* SELF, void* ARGS,
                                      Chuck_DL_Return* RETURN, Chuck_VM* VM,
                                      Chuck_VM_Shred* SHRED, CK_DL_API API);
typedef t_CKBOOL(CK_DLL_CALL* f_pmsg)(Chuck_Object* SELF, const char* MSG,
                                      void* ARGS, Chuck_VM* VM,
                                      Chuck_VM_Shred* SHRED, CK_DL_API API);
// uana specific
typedef t_CKBOOL(CK_DLL_CALL* f_tock)(Chuck_Object* SELF, Chuck_UAna* UANA,
...
typedef void(CK_DLL_CALL* f_shreds_watcher)(Chuck_VM_Shred* SHRED, t_CKINT CODE,
                                            t_CKINT PARAM, Chuck_VM* VM,
                                            void* BINDLE);
// type instantiation callback
typedef void(CK_DLL_CALL* f_callback_on_instantiate)(
  Chuck_Object* OBJECT, Chuck_Type* TYPE, Chuck_VM_Shred* originShred,
  Chuck_VM* VM);
}
...
typedef void(CK_DLL_CALL* f_setinfo)(Chuck_DL_Query* query, const char* key,
                                     const char* value);
// set info of module / chugin by (key,value)
typedef const char*(CK_DLL_CALL* f_getinfo)(Chuck_DL_Query* query,
                                            const char* key);

// begin class/namespace, can be nested
typedef void(CK_DLL_CALL* f_begin_class)(Chuck_DL_Query* query,
                                         const char* name, const char* parent);
// add constructor, must be betwen begin/end_class : use f_add_arg to add
...
typedef void(CK_DLL_CALL* f_add_mfun)(Chuck_DL_Query* query, f_mfun mfun,
                                      const char* type, const char* name);
// add static function - args to follow
typedef void(CK_DLL_CALL* f_add_sfun)(Chuck_DL_Query* query, f_sfun sfun,
                                      const char* type, const char* name);
// add binary operator overload - args included
typedef void(CK_DLL_CALL* f_add_op_overload_binary)(
  Chuck_DL_Query* query, f_gfun gfun, const char* type, const char* op,
  const char* lhsType, const char* lhsName, const char* rhsType,
  const char* rhsName);
// add unary (prefix) operator overload - arg included
typedef void(CK_DLL_CALL* f_add_op_overload_prefix)(
  Chuck_DL_Query* query, f_gfun gfun, const char* type, const char* op,
  const char* argType, const char* argName);
// add unary (postfix) operator overload - args included
typedef void(CK_DLL_CALL* f_add_op_overload_postfix)(
  Chuck_DL_Query* query, f_gfun gfun, const char* type, const char* op,
  const char* argType, const char* argName);
// add member variable
typedef t_CKUINT(CK_DLL_CALL* f_add_mvar)(
  Chuck_DL_Query* query, const char* type, const char* name,
  t_CKBOOL is_const); // TODO: public/protected/private
// add static variable
typedef void(CK_DLL_CALL* f_add_svar)(
  Chuck_DL_Query* query, const char* type, const char* name, t_CKBOOL is_const,
  void* static_addr); // TODO: public/protected/private
// add arg - follows ctor mfun sfun
typedef void(CK_DLL_CALL* f_add_arg)(Chuck_DL_Query* query, const char* type,
                                     const char* name);
// ** functions for adding unit generators, must extend ugen
typedef void(CK_DLL_CALL* f_add_ugen_func)(Chuck_DL_Query* query, f_tick tick,
                                           f_pmsg pmsg, t_CKUINT num_in,
                                           t_CKUINT num_out);
typedef void(CK_DLL_CALL* f_add_ugen_funcf)(Chuck_DL_Query* query,
                                            f_tickf tickf, f_pmsg pmsg,
                                            t_CKUINT num_in, t_CKUINT num_out);
typedef void(CK_DLL_CALL* f_add_ugen_funcf_auto_num_channels)(
  Chuck_DL_Query* query, f_tickf tickf, f_pmsg psmg);
// ** add a ugen control (not used) | 1.4.1.0 removed
// typedef void (CK_DLL_CALL * f_add_ugen_ctrl)( Chuck_DL_Query * query, f_ctrl
// ctrl, f_cget cget,
//                                              const char * type, const char *
//                                              name );
// end class/namespace - must correspondent with begin_class.  returns false on
// error
typedef t_CKBOOL(CK_DLL_CALL* f_end_class)(Chuck_DL_Query* query);
// create main thread hook- used for executing a "hook" function in the main
// thread of a primary chuck instance
typedef Chuck_DL_MainThreadHook*(CK_DLL_CALL* f_create_main_thread_hook)(
  Chuck_DL_Query* query, f_mainthreadhook hook, f_mainthreadquit quit,
  void* bindle);
// register a callback to be called on host shutdown, e.g., for chugin cleanup
typedef void(CK_DLL_CALL* f_register_callback_on_shutdown)(
  Chuck_DL_Query* query, f_callback_on_shutdown cb, void* bindle);
// register a callback function to receive notification from the VM about shreds
// (add, remove, etc.)
typedef void(CK_DLL_CALL* f_register_shreds_watcher)(Chuck_DL_Query* query,
                                                     f_shreds_watcher cb,
                                                     t_CKUINT options,
                                                     void* bindle);
// unregister a shreds notification callback
typedef void(CK_DLL_CALL* f_unregister_shreds_watcher)(Chuck_DL_Query* query,
                                                       f_shreds_watcher cb);

// documentation
// set current class documentation
typedef t_CKBOOL(CK_DLL_CALL* f_doc_class)(Chuck_DL_Query* query,
                                           const char* doc);
// add example of current class
typedef t_CKBOOL(CK_DLL_CALL* f_add_example)(Chuck_DL_Query* query,
                                             const char* ex);
// set current function documentation
typedef t_CKBOOL(CK_DLL_CALL* f_doc_func)(Chuck_DL_Query* query,
                                          const char* doc);
// set last mvar documentation
typedef t_CKBOOL(CK_DLL_CALL* f_doc_var)(Chuck_DL_Query* query,
                                         const char* doc);
...
struct Chuck_DL_Class {
    ...
    Chuck_DL_Class()
    {
        ...
        uana_tock           = NULL;
        ugen_pmsg           = NULL;
        current_mvar_offset = 0;
        ugen_num_in = ugen_num_out = 0;
    }
    // destructor
    ~Chuck_DL_Class();
};
...
struct Chuck_DL_Value {
    ...
    Chuck_DL_Value(const char* t, const char* n, t_CKBOOL c = FALSE,
                   void* a = NULL)
    {
        name        = n;
        type        = t;
...
union Chuck_DL_Return {
    ...
    Chuck_DL_Return()
    {
        v_vec4.x = v_vec4.y = v_vec4.z = v_vec4.w = 0;
    }
};
...
Chuck_DL_Value* make_new_svar(const char* t, const char* n, t_CKBOOL c,
                              void* a);
...
//-----------------------------------------------------------------------------
Chuck_String* CK_DLL_CALL ck_create_string(Chuck_VM* vm, const char* cstr,
                                           t_CKBOOL addRef);
//-----------------------------------------------------------------------------
...
Chuck_DL_Return CK_DLL_CALL ck_invoke_mfun_immediate_mode(
...
struct Chuck_DL_Api {
  ...
  public:
    ...
    static inline const Chuck_DL_Api* instance()
    {
        return &g_api;
    }
    ...
    struct VMApi {
        ...
        // queue an event; num_msg must be 1; buffer should be created using
        // create_event_buffer() above | 1.5.1.5
        t_CKBOOL(CK_DLL_CALL* const queue_event)(Chuck_VM* vm,
                                                 Chuck_Event* event,
                                                 t_CKINT num_msg,
                                                 CBufferSimple* buffer);
        // invoke Chuck_Object member function (defined either in chuck or c++)
        // | 1.5.1.5 (ge & andrew) NOTE this will call the member function in
        // IMMEDIATE MODE, marking it as a time-critical function when called in
        // this manner; any time/event operations therein will throw an
        // exception
        Chuck_DL_Return(CK_DLL_CALL* const invoke_mfun_immediate_mode)(
          Chuck_Object* obj, t_CKUINT func_vt_offset, Chuck_VM* vm,
          Chuck_VM_Shred* shred, Chuck_DL_Arg* ARGS, t_CKUINT numArgs);
        // throw an exception; if shred is passed in, it will be halted
        void(CK_DLL_CALL* const throw_exception)(const char* exception,
                                                 const char* desc,
    ...
    struct ObjectApi {
        ...
        // 1.5.0.0 (nshaheed and ge and anonymous pr-lab member) | changed from
        // private to public also changed all std::string & in this section to
        // const char * intent: this allows for chugins to access member
        // variables and create chuck strings
      public:
        ...
        // if addRef == TRUE the newly created object will have a reference
        // count of 1; otherwise 0 NOTE set addRef to TRUE if you intend to keep
        // a reference of the newly created object around (e.g., in the chugin)
        // NOTE set addRef to FALSE if the created object is to be returned
        // without keeping a reference around
        Object(CK_DLL_CALL* const create)(Chuck_VM_Shred*, Type type,
                                          t_CKBOOL addRef);
        // instantiating and initializing a ChucK object by type, using a VM
        // instead of a shred
        Object(CK_DLL_CALL* const create_without_shred)(Chuck_VM*, Type type,
                                                        t_CKBOOL addRef);
        // instantiate and initialize a ChucK string by type (without ref to a
        // parent shred)
        String(CK_DLL_CALL* const create_string)(Chuck_VM*, const char* value,
                                                 t_CKBOOL addRef);
        // compute pointer to data segment + offset; use for member variable
        // access | 1.5.2.0
        ...
        // set the origin shred; this should only be invoked by system-level
        // chugins; use with care
        void(CK_DLL_CALL* const set_origin_shred)(Object object,
                                                  Chuck_VM_Shred* shred);
        // function pointers for get_mvar_*()
        t_CKBOOL(CK_DLL_CALL* const get_mvar_int)(Object object,
                                                  const char* name,
                                                  t_CKINT& value);
        t_CKBOOL(CK_DLL_CALL* const get_mvar_float)(Object object,
                                                    const char* name,
                                                    t_CKFLOAT& value);
        t_CKBOOL(CK_DLL_CALL* const get_mvar_dur)(Object object,
                                                  const char* name,
                                                  t_CKDUR& value);
        t_CKBOOL(CK_DLL_CALL* const get_mvar_time)(Object object,
                                                   const char* name,
                                                   t_CKTIME& value);
        t_CKBOOL(CK_DLL_CALL* const get_mvar_vec2)(
          Object object, const char* name,
          t_CKVEC2& value); // 1.5.2.0 | vec2/complex/polar
        t_CKBOOL(CK_DLL_CALL* const get_mvar_vec3)(
          Object object, const char* name, t_CKVEC3& value); // 1.5.2.0 | vec3
        t_CKBOOL(CK_DLL_CALL* const get_mvar_vec4)(
          Object object, const char* name, t_CKVEC4& value); // 1.5.2.0 | vec4
        t_CKBOOL(CK_DLL_CALL* const get_mvar_string)(Object object,
                                                     const char* name,
                                                     String& value);
        t_CKBOOL(CK_DLL_CALL* const get_mvar_object)(Object object,
                                                     const char* name,
                                                     Object& value);
        // function pointer for set_string()
        t_CKBOOL(CK_DLL_CALL* const set_string)(String string,
                                                const char* value);
        // get a chuck string's internal c-string (NOTE do not save a reference
        // to the return value; make a copy if needed) | 1.5.2.0
        ...
        t_CKINT(CK_DLL_CALL* const array_int_get_idx)(ArrayInt array,
                                                      t_CKINT idx);
        void(CK_DLL_CALL* const array_int_set_idx)(ArrayInt array, t_CKINT idx,
                                                   t_CKINT value);
        t_CKBOOL(CK_DLL_CALL* const array_int_get_key)(ArrayInt array,
                                                       const char* key,
                                                       t_CKINT& value);
        void(CK_DLL_CALL* const array_int_set_key)(ArrayInt array,
                                                   const char* key,
                                                   t_CKINT value);
        t_CKBOOL(CK_DLL_CALL* const array_int_push_back)(ArrayInt array,
                                                         t_CKINT value);
        ...
        t_CKFLOAT(CK_DLL_CALL* const array_float_get_idx)(ArrayFloat array,
                                                          t_CKINT idx);
        void(CK_DLL_CALL* const array_float_set_idx)(ArrayFloat array,
                                                     t_CKINT idx,
                                                     t_CKFLOAT value);
        t_CKBOOL(CK_DLL_CALL* const array_float_get_key)(ArrayFloat array,
                                                         const char* key,
                                                         t_CKFLOAT& value);
        void(CK_DLL_CALL* const array_float_set_key)(ArrayFloat array,
                                                     const char* key,
                                                     t_CKFLOAT value);
        t_CKBOOL(CK_DLL_CALL* const array_float_push_back)(ArrayFloat array,
                                                           t_CKFLOAT value);
        ...
        t_CKVEC2(CK_DLL_CALL* const array_vec2_get_idx)(ArrayVec2 array,
                                                        t_CKINT idx);
        void(CK_DLL_CALL* const array_vec2_set_idx)(ArrayVec2 array,
                                                    t_CKINT idx,
                                                    t_CKVEC2 value);
        t_CKBOOL(CK_DLL_CALL* const array_vec2_get_key)(ArrayVec2 array,
                                                        const char* key,
                                                        t_CKVEC2& value);
        void(CK_DLL_CALL* const array_vec2_set_key)(ArrayVec2 array,
                                                    const char* key,
                                                    t_CKVEC2 value);
        t_CKBOOL(CK_DLL_CALL* const array_vec2_push_back)(
          ArrayVec2 array, const t_CKVEC2& value);
        ...
        t_CKVEC3(CK_DLL_CALL* const array_vec3_get_idx)(ArrayVec3 array,
                                                        t_CKINT idx);
        void(CK_DLL_CALL* const array_vec3_set_idx)(ArrayVec3 array,
                                                    t_CKINT idx,
                                                    t_CKVEC3 value);
        t_CKBOOL(CK_DLL_CALL* const array_vec3_get_key)(ArrayVec3 array,
                                                        const char* key,
                                                        t_CKVEC3& value);
        void(CK_DLL_CALL* const array_vec3_set_key)(ArrayVec3 array,
                                                    const char* key,
                                                    t_CKVEC3 value);
        t_CKBOOL(CK_DLL_CALL* const array_vec3_push_back)(
          ArrayVec3 array, const t_CKVEC3& value);
        ...
        t_CKVEC4(CK_DLL_CALL* const array_vec4_get_idx)(ArrayVec4 array,
                                                        t_CKINT idx);
        void(CK_DLL_CALL* const array_vec4_set_idx)(ArrayVec4 array,
                                                    t_CKINT idx,
                                                    t_CKVEC4 value);
        t_CKBOOL(CK_DLL_CALL* const array_vec4_get_key)(ArrayVec4 array,
                                                        const char* key,
                                                        t_CKVEC4& value);
        void(CK_DLL_CALL* const array_vec4_set_key)(ArrayVec4 array,
                                                    const char* key,
                                                    t_CKVEC4 value);
        t_CKBOOL(CK_DLL_CALL* const array_vec4_push_back)(
          ArrayVec4 array, const t_CKVEC4& value);
        void(CK_DLL_CALL* const array_vec4_clear)(ArrayVec4 array);
        // (UNSAFE) get c++ vector pointers from chuck arrays | 1.5.2.0
        // std::vector<t_CKUINT> * (CK_DLL_CALL * const array_int_vector)(
        // ArrayInt array ); std::vector<t_CKFLOAT> * (CK_DLL_CALL * const
        // array_float_vector)( ArrayFloat array ); std::vector<t_CKVEC2> *
        // (CK_DLL_CALL * const array_vec2_vector)( ArrayVec2 array );
        // std::vector<t_CKVEC3> * (CK_DLL_CALL * const array_vec3_vector)(
        // ArrayVec3 array ); std::vector<t_CKVEC4> * (CK_DLL_CALL * const
        // array_vec4_vector)( ArrayVec4 array );
    }* const object;
    ...
    struct TypeApi {
        ...
        t_CKINT(CK_DLL_CALL* const get_vtable_offset)(Chuck_VM*, Type type,
                                                      const char* funcName);
        ...
        // register a callback to be invoked whenever a base-type (or its
        // subclass) is instantiated, with option for type system to auto-set
        // shred origin if available
        void(CK_DLL_CALL* const callback_on_instantiate)(
          f_callback_on_instantiate callback, Type base_type, Chuck_VM* vm,
        ...
        // get type name (full, with decorations) (NOTE do not save a reference
        // to the return value; make a copy if needed) | 1.5.2.0
        const char*(CK_DLL_CALL* const name)(Type type);
        // get type base name (no decorations) (NOTE do not save a reference to
        // the return value; make a copy if needed) | 1.5.2.0
  ...
  private:
    ...
    Chuck_DL_Api(Chuck_DL_Api& a)
        : vm(a.vm), object(a.object), type(a.type), shred(a.shred)
    {
        assert(0);
    };
    // make this object un-copy-able, part 2
    Chuck_DL_Api& operator=(Chuck_DL_Api& a)
    {
        assert(0);
        return a;
    }
...
struct Chuck_DLL /* : public Chuck_VM_Object */
{
  ...
  public:
    ...
    Chuck_DLL(Chuck_Carrier* carrier, const char* xid = NULL)
        ...
        , m_id(xid ? xid : "")
        , m_done_query(FALSE)
        , m_api_version_func(NULL)
        , m_info_func(NULL)
        , m_query_func(NULL)
        , m_query(carrier, this)
        , m_apiVersionMajor(0)
        , m_apiVersionMinor(0)
    {
    }
