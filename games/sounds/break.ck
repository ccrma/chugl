...
// TODO just make own sndbuf around the okay-audio-format and wav files
// see pl-synth for simplified wav file writer
SndBuf amen("/Users/Andrew/Downloads/amen.wav") => dac;
1 => amen.loop;

// load samples into buff
float sample_buffer[amen.samples()];
for (int i; i < sample_buffer.size(); i++) {
    amen.valueAt(i) => sample_buffer[i];
}
