...
@group(1) @binding(1) var<storage> u_polygon_vertices : array<vec2f>; // x1, y1, x2, y2 ...
...
@group(1) @binding(6) var<uniform> u_antialias: i32; 
...
fn vs_main(
    ...
    let polygon_transform = vec4f(u_polygon_transforms[polygon_instance_idx]);
    ...
    for (var i = 0; i < polygon_vertex_count; i++) {
        f_points[i] = invScale * (
            u_polygon_vertices[(polygon_vertex_offset + i)] - center
        );
    }
...
fn fs_main(in : VertexOutput) -> @location(0) vec4f {
    ...
    if (bool(u_antialias)) {
        ...
        alpha = in.f_color.a * smoothstep(in.f_radius, in.f_radius - aaf, sdf);
    } else {
        alpha = step(sdf, 0.0);
    }
    ...
    return vec4f(in.f_color.rgb, alpha);
}