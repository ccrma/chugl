 ...
 *
 * You can add rigid bodies and joint constraints to the world and run the
 * simulation. You can get contact information to get contact points and normals
 * as well as events. You can query to world, checking for overlaps and casting
 * rays or shapes. There is also debugging information such as debug draw,
 * timing information, and counters. You can find documentation here:
 * https://box2d.org/
 * @{
 */

/// Create a world for rigid body simulation. A world contains bodies, shapes,
/// and constraints. You make create
///	up to 128 worlds. Each world is completely independent and may be
...
/// can increase accuracy. Typically 4.
...
/// Do not store a reference to this data.
B2_API b2BodyEvents b2World_GetBodyEvents(b2WorldId worldId);

/// Get sensor events for the current time step. The event data is transient. Do
/// not store a reference to this data.
...
B2_API void b2World_OverlapAABB(b2WorldId worldId, b2AABB aabb,
                                b2QueryFilter filter, b2OverlapResultFcn* fcn,
                                void* context);
...
/// point, or n-points. The ray-cast ignores shapes that contain the starting
/// point.
///	@param worldId The world to cast the ray against
///	@param origin The start point of the ray
///	@param translation The translation of the ray from the start point to
///the end point
///	@param filter Contains bit flags to filter unwanted shapes from the
///results
/// @param fcn A user implemented callback function
/// @param context A user context that is passed along to the callback function
///	@note The callback function may receive shapes in any order
B2_API void b2World_CastRay(b2WorldId worldId, b2Vec2 origin,
...
/// function. This is less general than b2World_CastRay() and does not allow for
/// custom filtering.
B2_API b2RayResult b2World_CastRayClosest(b2WorldId worldId, b2Vec2 origin,
                                          b2Vec2 translation,
                                          b2QueryFilter filter);

/// Cast a circle through the world. Similar to a cast ray except that a circle
...
/// very small
///	because it will prevent bodies from sleeping. Typically in meters per
///second.
///	@see b2WorldDef
...
B2_API void b2World_SetCustomFilterCallback(b2WorldId worldId,
                                            b2CustomFilterFcn* fcn,
                                            void* context);
...
///	@param impulse The impulse of the explosion, typically in kg * m / s or
///N * s.
B2_API void b2World_Explode(b2WorldId worldId, b2Vec2 position, float radius,
                            float impulse);
...
/// (non-dimensional)
/// @param pushVelocity The maximum contact constraint push out velocity (meters
/// per second)
///	@note Advanced feature
B2_API void b2World_SetContactTuning(b2WorldId worldId, float hertz,
                                     float dampingRatio, float pushVelocity);
...
/// retained. So you can create the definition
...
/// Get the linear velocity of a body's center of mass. Typically in meters per
/// second.
...
B2_API void b2Body_ApplyForce(b2BodyId bodyId, b2Vec2 force, b2Vec2 point,
                              bool wake);
...
/// linear velocity.
///	This optionally wakes the body. The torque is ignored if the body is not
///awake.
///	@param bodyId The body id
...
B2_API void b2Body_ApplyLinearImpulse(b2BodyId bodyId, b2Vec2 impulse,
                                      b2Vec2 point, bool wake);
...
///force,
/// use a force instead, which will work better with the sub-stepping solver.
B2_API void b2Body_ApplyLinearImpulseToCenter(b2BodyId bodyId, b2Vec2 impulse,
                                              bool wake);
...
///force,
/// use a force instead, which will work better with the sub-stepping solver.
B2_API void b2Body_ApplyAngularImpulse(b2BodyId bodyId, float impulse,
                                       bool wake);
...
/// using the
///	shape geometry and density. This information is lost if a shape is added
///or removed or if the 	body type changes.
...
/// Set the automatic mass setting. Normally this is set in b2BodyDef before
/// creation.
...
///touching this body to sleep, 	which can be expensive and possibly unintuitive.
...
/// Set this body to have fixed rotation. This causes the mass to be reset in
/// all cases.
...
B2_API int b2Body_GetShapes(b2BodyId bodyId, b2ShapeId* shapeArray,
                            int capacity);
...
B2_API int b2Body_GetJoints(b2BodyId bodyId, b2JointId* jointArray,
                            int capacity);
...
/// this may not encompass the body origin.
 ...
 * Shapes bind raw geometry to bodies and hold material properties including
...
B2_API b2ShapeId b2CreateCircleShape(b2BodyId bodyId, const b2ShapeDef* def,
                                     const b2Circle* circle);

/// Create a line segment shape and attach it to a body. The shape definition
/// and geometry are fully cloned. Contacts are not created until the next time
/// step.
///	@return the shape id for accessing the shape
B2_API b2ShapeId b2CreateSegmentShape(b2BodyId bodyId, const b2ShapeDef* def,
                                      const b2Segment* segment);

/// Create a capsule shape and attach it to a body. The shape definition and
/// geometry are fully cloned. Contacts are not created until the next time
/// step.
///	@return the shape id for accessing the shape
B2_API b2ShapeId b2CreateCapsuleShape(b2BodyId bodyId, const b2ShapeDef* def,
...
/// step.
///	@return the shape id for accessing the shape
B2_API b2ShapeId b2CreatePolygonShape(b2BodyId bodyId, const b2ShapeDef* def,
...
///	and must be carefully handled due to multithreading. Ignored for
///sensors.
///	@see b2PreSolveFcn
...
B2_API b2CastOutput b2Shape_RayCast(b2ShapeId shapeId, b2Vec2 origin,
                                    b2Vec2 translation);
...
/// otherwise returns b2_nullChainId.
...
/// Get the touching contact data for a shape. The provided shapeId will be
/// either shapeIdA or shapeIdB on the contact data.
B2_API int b2Shape_GetContactData(b2ShapeId shapeId, b2ContactData* contactData,
                                  int capacity);

/// Get the current world AABB
B2_API b2AABB b2Shape_GetAABB(b2ShapeId shapeId);

/// Get the closest point on a shape to a target point. Target and result are in
/// world space.
 ...
 * @brief Joints allow you to connect rigid bodies together while allowing
 * various forms of relative motions.
 * @{
 */
...
/// Enable/disable the distance joint spring. When disabled the distance joint
/// is rigid.
...
B2_API void b2DistanceJoint_SetSpringDampingRatio(b2JointId jointId,
                                                  float dampingRatio);
...
B2_API void b2DistanceJoint_SetLengthRange(b2JointId jointId, float minLength,
                                           float maxLength);
...
B2_API b2JointId b2CreateMotorJoint(b2WorldId worldId,
                                    const b2MotorJointDef* def);

/// Set the motor joint linear offset target
B2_API void b2MotorJoint_SetLinearOffset(b2JointId jointId,
                                         b2Vec2 linearOffset);
...
B2_API void b2MotorJoint_SetAngularOffset(b2JointId jointId,
                                          float angularOffset);
...
B2_API void b2MotorJoint_SetCorrectionFactor(b2JointId jointId,
 ...
 *
 * The mouse joint is designed for use in the samples application, but you may
 * find it useful in applications where the user moves a rigid body with a
 * cursor.
 * @{
 */
...
B2_API void b2MouseJoint_SetSpringDampingRatio(b2JointId jointId,
                                               float dampingRatio);
 ...
 * @brief A prismatic joint allows for translation along a single axis with no
 * rotation.
 *
 * The prismatic joint is useful for things like pistons and moving platforms,
 * where you want a body to translate along an axis and have no rotation. Also
 * called a *slider* joint.
 * @{
 */
...
/// This should usually be less than a quarter of the simulation rate. For
/// example, if the simulation runs at 60Hz then the joint stiffness should be
/// 15Hz or less.
...
B2_API void b2PrismaticJoint_SetSpringDampingRatio(b2JointId jointId,
                                                   float dampingRatio);
...
B2_API void b2PrismaticJoint_SetLimits(b2JointId jointId, float lower,
                                       float upper);
 ...
 * @brief A revolute joint allows for relative rotation in the 2D plane with no
 * relative translation.
 *
 * The revolute joint is probably the most common joint. It can be used for
 * ragdolls and chains. Also called a *hinge* or *pin* joint.
 * @{
 */
...
B2_API void b2RevoluteJoint_SetSpringDampingRatio(b2JointId jointId,
                                                  float dampingRatio);
...
B2_API void b2RevoluteJoint_SetLimits(b2JointId jointId, float lower,
                                      float upper);
 ...
 * @brief A weld joint fully constrains the relative transform between two
 * bodies while allowing for springiness
 *
 * A weld joint constrains the relative rotation and translation between two
 * bodies. Both rotation and translation can have damped springs.
 *
 * @note The accuracy of weld joint is limited by the accuracy of the solver.
 * Long chains of weld joints may flex.
 * @{
 */
...
B2_API void b2WeldJoint_SetLinearDampingRatio(b2JointId jointId,
                                              float dampingRatio);
...
B2_API void b2WeldJoint_SetAngularDampingRatio(b2JointId jointId,
                                               float dampingRatio);
 ...
 *
 * The wheel joint restricts body B to move along a local axis in body A. Body B
 * is free to rotate. Supports a linear spring, linear limits, and a rotational
 * motor.
 *
 * @{
 */
...
B2_API b2JointId b2CreateWheelJoint(b2WorldId worldId,
...
B2_API void b2WheelJoint_SetSpringDampingRatio(b2JointId jointId,
                                               float dampingRatio);
