...
{
    return hashmap_new_with_allocator(NULL, NULL, NULL, elsize, cap, seed0, seed1, hash,
...
bool hashmap_scan(struct hashmap* map, bool (*iter)(const void* item, void* udata),
                  void* udata)
{
    for (size_t i = 0; i < map->nbuckets; i++) {
        struct bucket* bucket = bucket_at(map, i);
...
static void all(void)
{
    ...
    while (!(map = hashmap_new(sizeof(int), 0, seed, seed, hash_int, compare_ints_udata,
                               NULL, NULL))) {
    }
    ...
    while (!(map = hashmap_new(sizeof(char*), 0, seed, seed, hash_str, compare_strs,
                               free_str, NULL)));
    ...
    if (total_allocs != 0) {
        fprintf(stderr, "[ChuGL:hashmap] total_allocs: expected 0, got %lu\n",
                total_allocs);
#ifdef CHUGL_DEBUG
        exit(1);
#endif
...
static void benchmarks(void)
{
    ...
    map = hashmap_new(sizeof(int), 0, seed, seed, hash_int, compare_ints_udata, NULL,
                      NULL);
    bench("set", N, {
    ...
    map = hashmap_new(sizeof(int), N, seed, seed, hash_int, compare_ints_udata, NULL,
                      NULL);
    bench("set (cap)", N, {
        const int* v = hashmap_set(map, &vals[i]);
        assert(!v);
