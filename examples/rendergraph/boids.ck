//-----------------------------------------------------------------------------
// name: boids.ck
// desc: boids! flocking with compute shaders
// 
...
// configure the rendergraph, adding a compute pass
...
// attaching shader to compute pass
...
Shader mat_shader(draw);
Material boids_material;
// set shader for material
boids_material.shader(mat_shader);
...
// add mesh to scene; contains entire flock of boids
GMesh boids_mesh(boids_geo, boids_material) --> GG.scene();

// for sending arrays to shaders
...
fun void ui()
{
    ...
    while (true)
    {
        ...
        // set UI window size
        UI.setNextWindowSize(@(400, 250), UI_Cond.Once);
        // begin UI
        if (UI.begin("Boids options", null, 0))
        {
            // slider for cohesion radius
            if (UI.slider("cohesion radius", cohesion_radius, 0.0, 0.5))
                compute_pass.uniformFloat(4, cohesion_radius.val());
            // slider for cohesion factor
...
fun void drawCursorAndUpdateVolume()
{
    ...
    while (true)
    {
        ...
        GG.nextFrame() => now;
        // get mouse cursor
        GG.camera().screenCoordToWorldPos(GWindow.mousePos(), 1.0) => vec3 mouse_cursor;
        // set pos
        mouse_cursor => cursor.pos;
        // update cursor position for shader
...
while (true)
{
    ...
    // synchronize #2 (ping-pong / double buffering)
    GG.nextFrame() => now;
    // bind
